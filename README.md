...
実行環境
=====
Windows 10の場合、ダウンロードしたzipファイルを右クリックして`プロパティ`から`ブロックの解除`にチェックを入れて`OK`押す。その後、zipファイルを再び右クリックして`すべて展開`から`展開`を押す。`説明書`フォルダ内の`Manual.txt`にストーリーや操作説明が記載してあります。`Game2.exe`を実行するとゲームが開始されます。

...
実行環境
=====
Windows 10の場合、ダウンロードしたzipファイルを右クリックして`プロパティ`から`ブロックの解除`にチェックを入れて`OK`押してください。その後、zipファイルを再び右クリックして`すべて展開`から`展開`を押してください。`説明書`フォルダ内の`Manual.txt`にストーリーや操作説明が記載してあります。`Game2.exe`を実行するとゲームが開始されます。

Windows Vista/7/10で動作することが確認されています。ただし、VistaではBGMが演奏されません。
